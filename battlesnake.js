...
color = ["#5ECBC2", "#E80978", "#3E338F", "#4C89C8"];
...
const pickDirection = (directions, me, board) => {
  ...
  return recommendedDirections.length > 0
    ? recommendedDirections[Math.floor(Math.random() * recommendedDirections.length)]
...
const checkSpot = (board, snakes, me, position) => {
  if (
    ...
    const snake = _.find(snakes, (snake) => board[position.x][position.y].item === snake.id);
    if (!snake) {
      return false;
...
const assessSpot = (board, me, position) => {
  ...
  const snake = _.find(board.snakes, (snake) =>
    _.some(snake.body, (piece) => piece.x === position.x && piece.y === position.y)
  );
